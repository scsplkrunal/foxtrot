<?php
	class client_suitebility_master extends db{
		...
		public function insert_update_horizon($data){
			...
			if($option==''){
          	...
          	if($this->errors!=''){
				...
				$q = "SELECT * FROM `".$this->$table_horizon."` WHERE `is_delete`='0' AND `option`='".$option."' ".$con;
				$res = $this->re_db_query($q);
				$return = $this->re_db_num_rows($res);
				if($return>0){
					$this->errors = 'This range is already exists.';
				...
				if($this->errors!=''){
					...
					if($id==0){
						$q = "INSERT INTO `".$this->$table_horizon."` SET `option`='".$option."'".$this->insert_common_sql();
						$res = $this->re_db_query($q);
                        $id = $this->re_db_insert_id();
						if($res){
						    $_SESSION['success'] = INSERT_MESSAGE;
						...
						$q = "UPDATE `".$this->$table_horizon."` SET `option`='".$option."' ".$this->update_common_sql()." WHERE `id`='".$id."'";
						$res = $this->re_db_query($q);
						if($res){
						    $_SESSION['success'] = UPDATE_MESSAGE;
							return true;
		...
		public function delete_horizon($id){
			$id = trim($this->re_db_input($id));
			if($id>0 && ($status==0 || $status==1) ){
				$q = "UPDATE `".$this->$table_horizon."` SET `is_delete`='1' WHERE `id`='".$id."'";
				$res = $this->re_db_query($q);
				if($res){
				    $_SESSION['success'] = DELETE_MESSAGE;
					return true;
