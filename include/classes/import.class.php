            ...
            if($id > 0)
            {
                ...
                while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
                 ...
                 if(isset($file_type_check) && ($file_type_check == '07' || $file_type_check == '08' || $file_type_check == '09'))
                 {
                     ...
                     //generate exception on file data
                     $check_fanmail_array = $this->get_fanmail_detail_data($id);
                     foreach($check_fanmail_array as $check_data_key=>$check_data_val)
                     {
                        if($check_data_val['brokerage_identification_number'] != '' && $check_data_val['dealer_branch_number'] != '' && $check_data_val['social_security_number'] != '' && $check_data_val['cusip_number'] != '' && $check_data_val['brokerage_identification_number(BIN)'] != '' && $check_data_val['customer_date_of_birth'] != '' )
                        {}
